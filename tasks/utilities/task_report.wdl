...
task make_pdf {
  ...
  command <<<
    ...
    combined_html_output_file = "~{report_name}"

    
    # loop through each individual report
    for file in "~{sep="*" individual_reports}".split("*"):
      table = pd.read_csv(file, delimiter='\t', header=0, dtype={"sample": 'str'}) # ensure sample_id is always a string

      # write out a title using the samplename
      single_html = "<!DOCTYPE html><html><h1>" + table["sample"] + "</h1>"
      single_html += "<body><h2>Report generated by " + table["analyst_name"] + " on " + str(date.today().isoformat()) + "</h2>"
      single_html += "<h3>Organism: " + table["gambit_predicted_taxon"] + "</h3>"
      single_html += "<h3>QC metrics:</h3>"
