...
task make_individual_report {
  ...
  command <<<
    ...
    all_columns = hidden_outputs + run_ids + standard_outputs + workflow_version_outputs + default_qc + additional_qc + additional_cols + organism_specific
    ...
    for column in final_columns:
      if column in row.columns:
        ...
        elif column in run_ids:
          output_type[column] = ["runid"]
        else:
...
task make_pdf {
  ...
  command <<<
    ...
    # indicate the desired style parameters for the HTML output
    html_output += "<style>table{border-collapse: collapse;width: 100%;} th, td{text-align: left; padding: 4px;} td{overflow-wrap: break-word; word-break: break-all} tr:nth-child(even){background-color: #EDEDED;} .page-break-after{page-break-after:always;} .justified-text{text-align:justify;} p{line-height: 0.5;} @media print{.page-break-after:last-child{page-break-after:avoid;}}</style>"
    ...
    for entity in samples:
      ...
      version_information = "<p>Workflow version and analysis date "
      run_id = "<p> Run ID: "
      ...
      for column in table.columns:
        if types[column][0] == "qc":
          ...
          version_information += "- <b>" + str(table[column][0]) + "</b> "
        elif types[column][0] == "runid":
          run_id += "<b>" + str(table[column][0]) + "</b><p>"
      
      version_information += "</p>"
      
      html_output += "<div class=\"page-break-after\"><h1 style=\"display:inline-block;margin-bottom:2px;\">" + table["sample"][0] + "</h1>"
      html_output += "<p style=\"display:inline-block;text-align:right;float:right;line-height:2;\">Analyst:_________________ Date:_________________<br>Supervisor:_________________ Date:_________________</p>"
      html_output += "<p><br><br><br>Report generated by <b>" + table["analyst_name"][0] + "</b> on <b>" + str(date.today().isoformat()) + "</b></p>"
      html_output += version_information
      html_output += run_id

      html_output += "<h2>Predicted organism: " + table["gambit_predicted_taxon"][0] + "</h2>"

      html_output += "<h3>QC metrics:</h3>"
      html_output += "<table style=\"width: auto\">" + qc_html_table + "</table>"

      html_output += "<h3>Standard outputs:</h3>"
      html_output += "<table style=\"width: auto\">" + standard_html_table + "</table>"
