...
task make_pdf {
  ...
  command <<<
    ...
    html_output += "<style>table{border-collapse: collapse;width: 100%;} th, td{text-align: left; padding: 4px;} td{overflow-wrap: break-word; word-break: break-all} tr:nth-child(even){background-color: #EDEDED;} .page-break-after{page-break-after:always;}</style>"
    ...
    for entity in samples:
      ...
      html_output += "<p>Report generated by " + table["analyst_name"][0] + " on " + str(date.today().isoformat()) + "</p>"
      html_output += "<h2>Organism: " + table["gambit_predicted_taxon"][0] + "</h2>"
      ...
      for column in table.columns:
        if types[column][0] == "qc":
          ...
          version_information += "- " + str(table[column][0]) + " "
